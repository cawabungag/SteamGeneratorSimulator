...
namespace OPCClientInterface
{
	public class OPCUAClientEX
	{
		...
		public async void UpdateInTime(string key,
			object TagNodeTag,
			...
			try
			{
				...
				else
				{
					...
					try
					{
						...
						{
							// 显示单个数本身                       
							var node = AddDataToList(ReadOneNodeFiveAttributes(new List<NodeId>() {sourceId}), 0,
								index++, sourceId);
							opcList.Add(node);
						}
					}
				...
				dataTable = await Task.Run(() =>
				{
					...
					else
					{
						...
						try
						{
							...
							{
								// 显示单个数本身                       
								AddDataTableNewRow(ref dataTable,
									ReadOneNodeFiveAttributes(new List<NodeId>() {sourceId}), 0, index++, sourceId);
							}
						}
						catch (Exception exception)
						{
		...
		/// <param name="index"></param>
		/// <param name="nodeId"></param>
		private void AddDataTableNewRow(ref DataTable dataTable,
